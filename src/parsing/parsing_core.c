...
/*   Updated: 2020/03/12 19:10:28 by coscialp         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int		parsing_analyser(char *line)
{
	if (line[0] == 'F')
	{
		if (data()->col_floor != -1 || !(parsing_color(line)))
			return (msg_error("color"));
	}
	else if (line[0] == 'C')
	{
		if (data()->col_ceil != -1 || !(parsing_color(line)))
			return (msg_error("color"));
	}
...
void	parsing_core(t_cub3d *c)
{
	...
	while (get_next_line(data()->fd, &current_line) > 0)
	{
		...
		while (current_line[i] && ft_isspace(current_line[i]))
			i++;
		if (parsing_analyser(current_line + i) == -1)
		{
	...
	if ((parsing_map() == -1) || parsing_end(data(), map()) == -1)
		ft_exit(c);
}
